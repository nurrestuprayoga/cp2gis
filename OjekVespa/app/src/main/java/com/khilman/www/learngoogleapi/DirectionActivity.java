...
public class DirectionActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    private LatLng pickUpLatLng = new LatLng(-7.2754438,112.6416432); // Surabaya
    private LatLng locationLatLng = new LatLng(-7.454546,112.5252464); // Sidoarjo
    ...
    private void actionRoute() {
        String Penjemputan = pickUpLatLng.latitude + "," + pickUpLatLng.longitude;
        String Tujuan = locationLatLng.latitude + "," + locationLatLng.longitude;

        // Panggil Retrofit
        ApiServices api = InitLibrary.getInstance();
        // Siapkan request
        Call<ResponseRoute> routeRequest = api.request_route(Penjemputan, Tujuan, API_KEY);
        // kirim request
            ...
            public void onResponse(Call<ResponseRoute> call, Response<ResponseRoute> response) {
                ...
                if (response.isSuccessful()){
                    ...
                    mMap.addPolyline(new PolylineOptions().addAll(decodePath)
                                                            .width(8f).color(Color.argb(255, 0, 255, 9)))
                                                            .setGeodesic(true);

                    // Tambah Marker
                    mMap.addMarker(new MarkerOptions().position(pickUpLatLng).title("Penjemputan"));
                    mMap.addMarker(new MarkerOptions().position(locationLatLng).title("Tujuan"));
